...
FeatureUsages AS 
(
    SELECT * FROM {{ ref('stg_FeatureUsages') }} WHERE Feature='visual_editor'
),


FeatureUsagesUsed AS 
(
    SELECT * FROM {{ ref('stg_FeatureUsages') }} WHERE Feature='visual_editor' AND Date>(current_date-16)
)
...
CASE WHEN (SUM(FeatureUsagesUsed.usage))>10 THEN 'Yes' ELSE 'No' END AS Used,
FROM Applications
